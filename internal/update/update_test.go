...
func TestGen0CleanClaim(t *testing.T) {
	...
	for _, want := range []string{"template_version: 6", "primary: claude-fable-5",
	...
	if !strings.HasPrefix(cl.newContent, "<!-- spine:begin v6 -->") {
...
func TestUnrecognizedEditsSkipUnlessForce(t *testing.T) {
	...
	if !strings.Contains(string(got), "template_version: 6") {
...
func TestAdoptModeSynthesizesWorkflow(t *testing.T) {
	...
	if !strings.Contains(wf.Diff, "profile: go-service") || !strings.Contains(wf.Diff, "template_version: 6") || !strings.Contains(wf.Diff, "# Workflow — praxis") {
...
func TestVersionDowngradeGuard(t *testing.T) {
	...
	bumped := strings.Replace(string(raw), "template_version: 6", "template_version: 7", 1)
	if bumped == string(raw) {
		t.Fatal("template_version: 6 not found in scaffolded WORKFLOW.md to bump")
	}
