...
var pinEvidenceToday = time.Date(2026, 8, 30, 18, 0, 0, 0, time.FixedZone("PDT", -7*60*60))
...
func TestPinEvidenceClassifiesSelectedRunFailures(t *testing.T) {
	tests := []struct {
		...
		{name: "missing eval doc", ref: pinEvidenceRef, mutate: func(t *testing.T, dir string) { mustRemove(t, filepath.Join(dir, "docs", "evals", pinEvidenceEval, "eval.md")) }, want: PinEvidenceMissing, wantPath: "docs/evals/2026-08-30-routing-check/eval.md"},
		...
		{name: "future date", ref: pinEvidenceRef, mutate: func(t *testing.T, dir string) { writePinEvidenceRun(t, dir, "2026-08-31", pinEvidenceModel, fullPassingBattery()) }, want: PinEvidenceMalformed, wantPath: "docs/evals/2026-08-30-routing-check/runs/gpt-5-6-sol.md"},
		{name: "day 91 stale", ref: pinEvidenceRef, mutate: func(t *testing.T, dir string) { writePinEvidenceRun(t, dir, "2026-05-31", pinEvidenceModel, fullPassingBattery()) }, want: PinEvidenceStale, wantPath: "docs/evals/2026-08-30-routing-check/runs/gpt-5-6-sol.md"},
		{name: "case mismatch", ref: pinEvidenceRef, mutate: func(t *testing.T, dir string) { writePinEvidenceRun(t, dir, "2026-06-01", "GPT-5.6-SOL", fullPassingBattery()) }, want: PinEvidenceModelMismatch, wantPath: "docs/evals/2026-08-30-routing-check/runs/gpt-5-6-sol.md"},
		{name: "prefix mismatch", ref: pinEvidenceRef, mutate: func(t *testing.T, dir string) { writePinEvidenceRun(t, dir, "2026-06-01", pinEvidenceModel+"-preview", fullPassingBattery()) }, want: PinEvidenceModelMismatch, wantPath: "docs/evals/2026-08-30-routing-check/runs/gpt-5-6-sol.md"},
		...
		{name: "failed battery", ref: pinEvidenceRef, mutate: func(t *testing.T, dir string) { writePinEvidenceRun(t, dir, "2026-06-01", pinEvidenceModel, failingBattery()) }, want: PinEvidenceFailedBattery, wantPath: "docs/evals/2026-08-30-routing-check/runs/gpt-5-6-sol.md"},
		{name: "bad battery order", ref: pinEvidenceRef, mutate: func(t *testing.T, dir string) { writePinEvidenceRun(t, dir, "2026-06-01", pinEvidenceModel, strings.Replace(fullPassingBattery(), "invocation=KILLED,wiring=KILLED", "wiring=KILLED,invocation=KILLED", 1)) }, want: PinEvidenceMalformed, wantPath: "docs/evals/2026-08-30-routing-check/runs/gpt-5-6-sol.md"},
		{name: "inconsistent battery verdict", ref: pinEvidenceRef, mutate: func(t *testing.T, dir string) { writePinEvidenceRun(t, dir, "2026-06-01", pinEvidenceModel, strings.Replace(fullPassingBattery(), "battery_verdict: pass", "battery_verdict: fail", 1)) }, want: PinEvidenceMalformed, wantPath: "docs/evals/2026-08-30-routing-check/runs/gpt-5-6-sol.md"},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			dir := pinEvidenceRepo(t, "2026-06-01", pinEvidenceModel, fullPassingBattery())
			if tc.mutate != nil { tc.mutate(t, dir) }
...
func TestPinEvidenceDateQuoteAggregationAndContainment(t *testing.T) {
	t.Run("day 90 and quoted exact model pass", func(t *testing.T) {
		...
		if got := CheckPinEvidence(dir, []PinEvidencePin{{Key: "codex.primary", Model: pinEvidenceModel, EvidenceRefs: []string{pinEvidenceRef}}}, pinEvidenceToday); len(got) != 0 { t.Fatalf("findings = %#v", got) }
	})
	t.Run("all selected references are checked in byte order", func(t *testing.T) {
		...
		if len(got) != 1 || got[0].Kind != PinEvidenceMissing || !strings.HasSuffix(got[0].Path, "/a.md") { t.Fatalf("findings = %#v", got) }
	})
	...
	for _, rel := range []string{"docs", "docs/evals", "docs/evals/" + pinEvidenceEval, "docs/evals/" + pinEvidenceEval + "/runs", "docs/evals/" + pinEvidenceEval + "/runs/" + pinEvidenceRun + ".md"} {
		t.Run("symlink "+rel, func(t *testing.T) {
			...
			if err := os.Symlink(target, filepath.Join(dir, rel)); err != nil { t.Fatal(err) }
			...
			if len(got) != 1 || got[0].Kind != PinEvidenceMalformed || strings.Contains(got[0].Path, target) { t.Fatalf("findings = %#v", got) }
		})
	}
}
...
func writePinEvidenceRun(t *testing.T, dir, created, model, battery string) {
	t.Helper()
	extra := ""
	if battery != "" { extra = battery + "\n" }
	mustWrite(t, pinEvidenceRunPath(dir), "---\nname: "+pinEvidenceRun+"\ncreated: "+created+"\nmodel: "+model+"\nstage: raw\nscore: 1\n"+extra+"---\nsecret body\n")
}

func pinEvidenceRunPath(dir string) string { return filepath.Join(dir, "docs", "evals", pinEvidenceEval, "runs", pinEvidenceRun+".md") }

func fullPassingBattery() string { return "battery_version: 1\nbattery_verdict: pass\nbattery_results: invocation=KILLED,wiring=KILLED,flag-honoured=KILLED,column-presence=KILLED,column-order=KILLED,ordering=KILLED,units-labels=KILLED,security-default=REPORT-ONLY,lifecycle=REPORT-ONLY,error-path-behaviour=KILLED" }
func failingBattery() string { return strings.Replace(fullPassingBattery(), "battery_verdict: pass", "battery_verdict: fail", 1) }
func mustWrite(t *testing.T, path, content string) { t.Helper(); if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil { t.Fatal(err) }; if err := os.WriteFile(path, []byte(content), 0o644); err != nil { t.Fatal(err) } }
func mustRemove(t *testing.T, path string) { t.Helper(); if err := os.RemoveAll(path); err != nil { t.Fatal(err) } }
