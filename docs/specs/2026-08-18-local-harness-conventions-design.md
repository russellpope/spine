...
15. As a repo owner, I want `spine update` to create a `maipipe.toml`
    containing only maipipe's required top-level `schema = 0` and the region
    when none exists, so that adoption is additive. *(Amended 2026-08-19,
    I091: "only the region" is not a loadable maipipe file.)*
...
- Delivery region in `maipipe.toml`: `# spine:begin gate-pack go@1` …
  ...
  `schema = 0` plus the region when the file is absent (amended 2026-08-19,
  I091). Stage arrays are maipipe's singular `[[pipelines.<name>.stage]]`.
