...
**Override suspicion is value-granular; local-edit refusal is file-granular** (ruled 2026-07-25, recorded here per I039 review so later sweeps have something citable). `spine update`'s local-edit detector refuses a whole machine-owned file when *any* unrecognized edit appears anywhere in it — including edits unrelated to model routing (I039's live case: objectstudio's vfb pre-handoff gate prose stranded a byte-stock routing block at gen 9). A file-level refusal is therefore **not** evidence of a suspected model override. The sweep rule: diff the repo's routing values specifically against the pre-sweep baseline (or the shipped-default history) before deciding whether the raise-don't-apply gate attaches. Routing block byte-stock → hand-fold the file to the new generation's stock content, preserving the unrelated local edit verbatim; routing values genuinely divergent → the maipipe path, hand-migration with overrides retained and refresh questions raised to the owner. Without this rule, any unrelated local edit in a machine-owned file strands its repo at the prior generation indefinitely — by default rather than by decision.
...
**D9 — Value encoding.** An entry's value is the model id, optionally followed by ` @ <effort>`. **Parsing strips the trailing comment FIRST, then splits on the separator** — corrected 2026-07-24 after both the I033 and I036 reviews flagged the original wording. The original "split before comment-stripping" ordering is wrong: an `@` appearing inside a trailing comment (e.g. `# ask @russell`) would corrupt the parsed id. The shipped order is the robust one; treat any change reordering it as a regression, not a spec-conformance fix.
...
**D13 — Audit consumes the resolver.** The routing audit stops parsing `WORKFLOW.md` itself and calls the shared resolver, collapsing **three** independent parsers into one — `internal/model.readOverride`, `internal/update.ExtractKeys` and `internal/audit.readMapping`, which had differing block-termination behaviour (corrected from "two" 2026-07-24, I037 review RA2; the third was found during I035 review). Tier resolution becomes flavor-scoped, and alias matching moves from substring containment to explicit aliases declared per entry in the table.

**Reporting contract when the mirror is absent** (added 2026-07-24, I037 review RA1 — the original wording never said what an *upgraded* binary should report, and the gap was being filled implicitly). Resolution falls back to embedded defaults **and the report says so**, in both the missing-file and the missing-block cases. Verdicts are judged against those defaults, because that is what dispatch-time resolution actually returns — an audit reporting them unmapped would be lying about dispatch reality. This does not weaken user story 18: that story governs the **un-upgraded** binary reading a newer format, whose behaviour no change here can alter, and the upgraded binary's protection against a newer format is D14's hard refusal, which is stronger than a warning. Note the scope of AC "unmapped rather than guessing": it concerns a *token* mapping to no entry, not a *repo* missing its mirror.

**Alias/history matching is provenance-scoped** (added 2026-07-24, I037 review RA3/I-2). An `Override` entry matches on its exact id only: the aliases and historical ids shipped with the table describe the **displaced default**, not the owner's pin, so carrying them would judge a dispatch that ignored the override as a match — silencing the one signal that a deliberate pin was bypassed. `Default` and `Inherited` entries keep both. This scoping is load-bearing for the "scenario fixtures pass unchanged" criterion: all four fixtures carry an inherited `fallback: claude-opus-4-8`, and the mixed fixture's `opus` token resolves only via the Inherited entry's alias carry. A blanket strip breaks them; only the `Override`-scoped strip preserves them.
...
**D15 — Flavor of a dispatch.** Flavor is derived from the transcript source. While codex transcript parsing remains out of scope, this resolves to claude for every audited dispatch; the derivation point is named explicitly so the deferred codex-audit effort has a defined seam rather than a redesign. Where a model id is declared under more than one flavor, the transcript-derived flavor decides.

**Within a single flavor, one collision shape is sanctioned** (amended 2026-07-24, I037 review RA4 — the original "the table should avoid such collisions" was contradicted by the spec's own shipped data). A flavor's `fallback` entry legitimately names the same id as one of its ordered tiers, because D10 defines the codex fallback as "re-run on codex" — the shipped table declares `terra`/`gpt-5.6-terra` on both `codex.routine` and `codex.fallback` deliberately. Flavor scoping cannot disambiguate that, so within a flavor the resolution rule is: the declared tier if it is among the candidates, else the highest-ranked ordered candidate — the charitable reading where ambiguity exists, and the anti-laundering reading where it does not, so a shared id can never let real descent hide behind a lateral fallback interpretation. Known consequence for the deferred codex-audit effort: a FALLBACK record is consulted only when a token *resolves* to fallback, which on a shared ordered/fallback id it never will while an ordered candidate exists — so a properly recorded refusal-rerun of an above-tier ticket will judge silent-descent. That effort decides deliberately whether to consult FALLBACK records before tier resolution or to prefer the fallback reading when a record exists; it is a known edge, not a surprise.

**Second named edge for the same effort** (recorded 2026-07-25, final whole-branch review RA-B): D18's highest-tier-wins cluster rule up-tiers the lower-tier tickets sharing a cmux cluster. Once codex transcripts parse, those tickets' tokens will resolve above their declared tier with no ESCALATION record and judge warn-level `escalated-no-reason` under the audit's existing contract. Latent today (the transcript flavor seam is hard-wired claude), warn-level, never blocking — and arguably desirable visibility. The codex-audit effort inherits it alongside the FALLBACK-record edge above: same class of known consequence, decided deliberately later.

**D16 — Template generation 10.** The migration renders the flavor-axis mirror, retires the top-level `effort:` key, and stamps generation 10. A repo carrying a customized `effort:` value has it migrated into per-entry effort overrides on the repo's **claude** entries — the flavor the retired key governed; no gen ≤9 repo has codex rows, and blanketing codex would collide with its shipped effort suffixes (clarified 2026-07-24, I036 review RA2).

**"Customized"**, for both retired keys, means *differing from the shipped default of the file's own generation* — ADR 0002's choice-vs-default convention (clarified 2026-07-24, I036 review RA4).

**`model_default:` retires too** (ruled 2026-07-24, recorded here per I036 review RA3 so later tickets have something citable). It is a top-level key holding a model id that duplicates `model_routing.primary`, can silently diverge from it, and was verified to have **zero live consumers** — no Go code, no skill, nothing acts on it. Retirement is quiet when the on-disk value is either the generation's shipped default or equal to the resolved `claude.primary`. When the value is **both customized and divergent**, the migration refuses to write that file and names the conflict, directing the operator to adopt it as a `claude.primary` override or delete the line. The narrower "customized AND divergent" trigger is deliberate: surfacing *any* divergence would flag every pristine gen-0 repo, whose shipped `claude-opus-4-8` always differs from the resolved primary, stranding repos that made no choice. The retired key's prior emitted line joins the superseded-line set so it is recognized as machine-emitted rather than read as a local edit.
...
**The example-block convention is `EXAMPLE-ID`** (ratified 2026-07-25, I038 review RA1). "Outside documented example blocks" was undefined when written, which left the acceptance gate resting on a convention the implementation itself authored — circular. A line carrying a literal model id is exempt only if it also carries an `EXAMPLE-ID` marker comment naming why (historical smoke record, illustrative sample), and the regression test must **print every exempted line** on each run, so the exemption inventory is visible rather than tacit. The marker is deliberately not unforgeable: it makes exemptions auditable, not impossible.

**The preflight probes capability, not presence** (added 2026-07-25, I038 review I-2). `command -v spine` succeeds on a binary predating the `model` subcommand, whereupon resolution yields empty stdout and exit 2 — and an *unquoted* expansion of that empty result collapses the argument, letting the next token be eaten as the model argument. Skills therefore probe by running a real resolution, and every substitution into a spawn command is double-quoted so an empty result stays a distinct (loud) argument rather than vanishing.

**Worker resolution granularity follows the frontend's worker lifecycle** (clarified 2026-07-25, I038 review RA2/RA3). Per dispatch where workers are fresh per task (herdr); per task cluster at spawn where workers are long-lived and created before task assignment (cmux) — with a cluster's tier defined as the **highest** tier among its tasks, so a primary-tier ticket never resolves below primary. Scope: this governs the flavors whose dispatch lines name a model. claude-team's workers select via SDD's own capability heuristics, which carry no tier vocabulary and live upstream, out of scope here; bridging them to the table in claude-team's in-tree dispatch steps is a follow-up candidate, not part of this decision.
