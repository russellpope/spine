...
**D11 — New resolver module.** A new module exposes a pure resolution function taking a repo directory, a flavor, and a tier, and returning the resolved entry plus whether it came from a default or an override. This is the single new seam. Repo context comes from the working directory as with other spine commands, overridable by flag; outside a spine repo, resolution returns embedded defaults.
