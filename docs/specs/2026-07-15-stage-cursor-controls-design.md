...
- **/handoff hardening** (I014): the handoff skill must run `spine cursor` and embed its output verbatim — prose paraphrases of "what's next" are defined as incomplete. Backstops: doctor advisory + `audit stages` blocking check that the newest handoff document contains a cursor block whenever a cursor exists. (amended 2026-07-16, I025: the newest handoff's cursor block must also match the live cursor's effort — a stale-effort block is the same blocking finding)
