...
func TestVersionCommand(t *testing.T) {
	...
	if code != 0 || !strings.Contains(out, "spine template generation 4") {
...
func TestUpdateDryRunThenWrite(t *testing.T) {
	...
	code, out, _ = runCmd(t, "update", "--dir", dir)
	if code != 1 || !strings.Contains(out, "+ template_version: 4") {
...
func TestAdoptDryRunShowsDiffs(t *testing.T) {
	...
	if !strings.Contains(out, "+ template_version: 4") {
	...
	if strings.Contains(jsonOut, "+ template_version: 4\n") {
